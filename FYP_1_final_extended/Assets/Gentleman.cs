...
public class Gentleman : MonoBehaviour
{
    ...
    public string task = "";
    string input_text;
    String[] sentances_array;
    int arr_size = 0;
    int global_i = 0;
    public static bool action_completed = false;
    public void set_Action_completed()
    {
    ...
    public void OnSubmit()
    {
        ...
        // using the method 
        sentances_array = input_text.Split(spearator1,StringSplitOptions.None);

        arr_size = sentances_array.Length - 1;
        
        foreach(string s in sentances_array )
        {
            print(s);
        }
    }
    ...
    {
        StreamReader inp_stm = new StreamReader("D:/lectures/semmester#8/FYP_2/Project/FYP_1_final_extended_completed_1/FYP_1_final_extended/Assets/scrpit/Input.txt");

        while (!inp_stm.EndOfStream)
        {
            input_text = inp_stm.ReadLine();
    ...
    void Update()
    {
        if(action_completed && global_i <= arr_size)
        {
            get_text = sentances_array[global_i];

            // Updating Values of critical Variables
            global_i = global_i + 1;
            ...
            // Using the Method 
            String[] strlist = get_text.Split(spearator2, count, StringSplitOptions.None);
            ...
            else if (Subject == "housewife")
            {
                obj_HouseWife.OwnSubmit(get_text);
            }
            else if (Subject == "theif")
            {
                obj_Theif.OwnSubmit(get_text);
            }
            else if (Subject == "boy")
            {
                obj_boy.OwnSubmit(get_text);
            }
        }
